...
/datum/addiction/stimulants/lose_addiction(datum/mind/victim_mind)
	. = ..()
	victim_mind.current.remove_actionspeed_modifier(ACTIONSPEED_ID_STIMULANTS, TRUE)
	victim_mind.current.remove_status_effect(STATUS_EFFECT_WOOZY, TRUE)
	victim_mind.current.remove_movespeed_modifier(MOVESPEED_ID_STIMULANTS, TRUE)
